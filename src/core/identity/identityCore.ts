...
import { IdentityState, IdentitySnapshot, IdentityUpdate } from "./identityTypes";
...
export class IdentityCore {
  ...
  private driftMemory: number[] = [];

detectDrift(signal: any): boolean {
  ...
  const persistence =
    this.driftMemory.reduce((a, b) => a + b, 0) / this.driftMemory.length;

  // 🔥 FINAL DECISION (strict + memory gated)
  const weightedScore =
  (volatilityShock ? 0.4 : 0) +
  (fragility ? 0.35 : 0) +
