...
class HealingEngine {
    ...
    heal(reports) {
        ...
        const actions = degraded.map(d => ({
            target: d.component,
            action: d.severity === "CRITICAL"
                ? "FULL_RESET"
                : d.severity === "HIGH"
                    ? "RECALIBRATE"
                    : "SMOOTH_ADJUSTMENT",
            effect: d.severity === "CRITICAL"
                ? "RESET_STATE_VECTOR"
                : "REDUCE_NOISE"
        }));
        const overallHealth = 1 - (reports.reduce((s, r) => s + r.driftScore, 0) / reports.length);
        return {
            overallHealth,
