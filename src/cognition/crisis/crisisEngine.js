...
class CrisisEngine {
    ...
    computeFragility() {
        ...
        return nodes.map(n => {
            const connected = this.graph.getEdges().filter(e => e.from === n.id || e.to === n.id);
            const avgStrength = connected.reduce((sum, e) => sum + e.strength, 0) /
                (connected.length || 1);
    ...
    forecastCrisis() {
        ...
        const weakest = fragility.reduce((min, f) => f.fragilityScore > min.fragilityScore ? f : min, fragility[0]);
        const cascade = this.simulateCascade(weakest.instrument);
        const avgFragility = fragility.reduce((s, f) => s + f.fragilityScore, 0) / fragility.length;
...
exports.CrisisEngine = CrisisEngine;
