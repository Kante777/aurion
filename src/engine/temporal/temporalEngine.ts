...
export class TemporalEngine {
  ...
  private history = new Map<string, EventRecord[]>();

  private profiles = new Map<string, TemporalProfile>([
  ...
  addEvent(instrument: string, event: EventRecord) {
  ...
  getProfile(instrument: string): TemporalProfile {
  ...
  classifyInstrument(instrument: string) {
    ...
    if (profile.leadScore > 0.65) return "LEADER";
    if (profile.lagScore > 0.65) return "FOLLOWER";
   ...
   */
  temporalWeight(instrument: string) {
   ...
   */
  buildFlowOrder(instruments: string[]) {
   ...
   */
  detectFlowShift(a: string, b: string) {

    const aProfile = this.getProfile(a);
    const bProfile = this.getProfile(b);

    const aLeadsB =
      aProfile.leadScore > bProfile.leadScore &&
